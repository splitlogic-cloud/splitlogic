import { AdapterContext, CanonicalImportRow, ImportAdapter, NormalizedRow } from "../types";

function normHeader(value: string): string {
  return value.toLowerCase().trim().replace(/[^a-z0-9]+/g, "_");
}

function asString(v: string | undefined): string | null {
  const s = (v ?? "").trim();
  return s ? s : null;
}

function asNumber(v: string | undefined): number | null {
  const s = (v ?? "").trim();
  if (!s) return null;

  const normalized = s.replace(/\s+/g, "").replace(",", ".");
  const n = Number(normalized);
  return Number.isFinite(n) ? n : null;
}

function rowToObject(headers: string[], row: string[]): Record<string, unknown> {
  const out: Record<string, unknown> = {};
  headers.forEach((header, idx) => {
    out[normHeader(header)] = row[idx] ?? "";
  });
  return out;
}

function pickFirst(raw: Record<string, unknown>, aliases: string[]): string | undefined {
  for (const alias of aliases) {
    const value = raw[alias];
    if (typeof value === "string" && value.trim() !== "") {
      return value.trim();
    }
  }
  return undefined;
}

function toCanonical(
  ctx: AdapterContext,
  raw: Record<string, unknown>
): CanonicalImportRow {
  return {
    source_file_type: ctx.fileKind,
    source_name: "Believe",
    statement_period: asString(
      pickFirst(raw, ["statement_period", "sales_month", "period"])
    ),

    title: asString(
      pickFirst(raw, ["track_title", "title", "track_name"])
    ),
    artist: asString(
      pickFirst(raw, ["main_artist", "artist", "track_artist"])
    ),
    release_title: asString(
      pickFirst(raw, ["release_title", "album_title", "release"])
    ),

    isrc: asString(pickFirst(raw, ["isrc"])),
    upc: asString(pickFirst(raw, ["upc", "ean"])),
    territory: asString(pickFirst(raw, ["territory", "country"])),
    store: asString(pickFirst(raw, ["store", "service", "platform"])),

    quantity: asNumber(pickFirst(raw, ["quantity", "units", "sales_quantity"])),
    net_amount: asNumber(
      pickFirst(raw, ["net_receipts", "net_amount", "earnings", "amount"])
    ),
    gross_amount: asNumber(
      pickFirst(raw, ["gross_receipts", "gross_amount", "gross"])
    ),
    currency: asString(pickFirst(raw, ["currency", "account_currency"])),
    sale_date: asString(pickFirst(raw, ["sale_date", "transaction_date", "date"])),
    raw,
  };
}

export const believeAdapter: ImportAdapter = {
  key: "believe",
  displayName: "Believe adapter",

  canHandle(ctx) {
    const joined = ctx.headers.join(" ").toLowerCase();

    let score = 0;
    if (joined.includes("isrc")) score += 0.2;
    if (joined.includes("upc")) score += 0.2;
    if (joined.includes("main artist")) score += 0.2;
    if (joined.includes("track title")) score += 0.2;
    if (joined.includes("net receipts")) score += 0.2;

    return Math.min(score, 1);
  },

  normalize(ctx): NormalizedRow[] {
    const headers = ctx.headers.map(normHeader);
    const bodyRows = ctx.rows.slice(ctx.headerRowIndex + 1);

    return bodyRows
      .filter((row) => row.some((cell) => (cell ?? "").trim() !== ""))
      .map((row) => {
        const raw = rowToObject(headers, row);
        return {
          raw,
          canonical: toCanonical(ctx, raw),
        };
      });
  },
};