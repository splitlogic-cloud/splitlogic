...
export async function getMatchedRowsForAllocation(
  ...
  const allRows: MatchedImportRowForAllocation[] = [];
  let from = 0;
  ...
  while (true) {
    ...
    if (error) {
      throw new Error(`getMatchedRowsForAllocation failed: ${error.message}`);
    }

    const batch = (data ?? []) as MatchedImportRowForAllocation[];
...
export async function listImportRowsForAllocation(
  ...
  const allRows: ImportRowForAllocation[] = [];
  let from = 0;
