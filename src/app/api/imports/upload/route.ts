...
function normalizeSource(input: string | null): ImportSource {
  ...
  if (v === "revenue" || v.includes("revenue") || v.includes("royalty")) {
    return "revenue";
  }

  if (
    v === "masterdata" ||
    v.includes("masterdata") ||
    v.includes("works") ||
    v.includes("parties")
  ) {
    return "masterdata";
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const source = normalizeSource(
      String(form.get("source") ?? form.get("type") ?? "")
    );
    ...
    if (updErr) {
      return NextResponse.json(
        {
          ok: false,
          error: `Uploaded but failed to update job: ${updErr.message}`,
        },
        { status: 500 }
      );
    }
